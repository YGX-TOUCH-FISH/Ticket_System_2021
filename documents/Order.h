...
public:
    Order() = default;
    Order(const date &buyTime , int status, const String<21> &trainId, const String<40> &from, const String<40> &to,
...
private:
    ...
    BPlusTree<pair<String<21> , int> , pair<String<21> , Order>> pendingOrder;

public:
    Order_System(){
        userOrders_BPT.initialize("Order_BPT.dat" , "Order.dat");
        pendingOrder.initialize("pendingOrder_BPT.dat" , "pendingOrder.dat");//pendingOrder_BPT pair(TrainID , 班次数) —— pair(username , Order)
    }
    vector<Order> findOrder(const String<21> &username);
    vector<pair<String<21> , Order>> findPendingOrder(const pair<String<21> , int> &t);
    void addOrder(const String<21> &username, const Order &o);
    void refundOrder(const String<21> &username, const Order &o);
    void addPendingOrder(const String<21> &username, const Order &o , int no);
    void delPendingOrder(const String<21> &username, const Order &o , int no);
};
